...
trait DAOComponent {
  ...
  def filter(selectedCollectionFilter: Int,
             selectedSearchFilter: Int,
             search: String): Future[Seq[MovieDB]]
...
object DAO extends DAOComponent {
  ...
  private val db = Database.forURL(
      s"jdbc:h2:file:$localStore/tmdb-shelf", driver = "org.h2.Driver")
  ...
  override def updateSeenDate(
      tmdbId: Long, seenDate: java.sql.Date): Future[Int] = {
    val q = for { movie ← movies if movie.tmdbId === tmdbId } yield
      (movie.viewingDate, movie.seen)
    db.run(q.update((Some(seenDate), true)))
  }

  override def refreshMovie(tmdbId: Long, comment: String): Future[Int] = {
    val q = for { movie ← movies if movie.tmdbId === tmdbId } yield
      (movie.imdbScore, movie.comment)
