...
object Utils {
  ...
  case class Reset(shelf: org.edla.tmdb.shelf.TmdbPresenter,
                   items: Array[javafx.scene.image.ImageView])
  case class Search(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, search: String, user: Boolean)
  case class Position(x: Int, y: Int)
  case class AddPoster(shelf: org.edla.tmdb.shelf.TmdbPresenter,
                       poster: javafx.scene.image.ImageView)
  case class AddPosterXy(shelf: org.edla.tmdb.shelf.TmdbPresenter,
                         poster: javafx.scene.image.ImageView,
                         pos: Position)
  case class NotTheatricalFilmPoster(shelf: org.edla.tmdb.shelf.TmdbPresenter,
                                     poster: javafx.scene.image.ImageView)
  case class ShowPage(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, page: Int, maxPage: Int)
  case class RefreshMovieFromDb(shelf: org.edla.tmdb.shelf.TmdbPresenter,
                                title: String,
                                original_title: String,
                                release_date: String,
                                imdb_id: String)
  case class RefreshMovieFromTmdb(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, movie: Movie)
  case class RefreshCredits(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, tmdbId: Long, crew: Credits)
  ...
  case class ConfirmDeletion(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, movie: Movie)
  case class DeletionConfirmed(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, movie: Movie)
  case class ShowReleases(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, releases: Releases)
  case class ShowSeenDate(shelf: org.edla.tmdb.shelf.TmdbPresenter,
                          seenDate: Option[java.sql.Date],
                          comment: String)
  case class SaveSeenDate(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, seenDate: java.sql.Date)
  case class ShowPopup(shelf: org.edla.tmdb.shelf.TmdbPresenter, popup: String)
  case class SetCollectionFilter(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, filter: Number)
  case class SetSearchFilter(
      shelf: org.edla.tmdb.shelf.TmdbPresenter, filter: Number)
...
trait JfxUtils {
  ...
  def mkChangeListener[T](onChangeAction: (ObservableValue[_ <: T], T,
                                           T) ⇒ Unit): ChangeListener[T] = {
  ...
  def mkListChangeListener[E](
      onChangedAction: ListChangeListener.Change[_ <: E] ⇒ Unit)
  ...
  def mkCellFactoryCallback[T](listCellGenerator: ListView[T] ⇒ ListCell[T])
